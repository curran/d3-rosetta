<script>
  ...
  const setStateWrapper = (updaterFn) => {
    ...
    // The reactive block below ( $: ) will automatically call `viz` again due to `svelteState` changing.
  };
  ...
  $: if (containerElement && typeof viz === 'function') {
    viz(containerElement, { state: svelteState, setState: setStateWrapper });
  }
